...
const faultBits = [
  ...
  [1048576, "OVRT disconnection"],
  [2097152, "UVRT disconnection"],
  [4194304, "Aux LV overvoltage"],
  [8388608, "Aux LV undervoltage"],
  [16777216, "Emergency shutdown"],
  [33554432, "Device timeout"],
  [67108864, "DCDC/PFC communication loss"],
  [134217728, "DCDC/PFC communication errors"],
...
const monitoredProtectionGroups = [
  "Over/under voltage",
  "Over current",
  "Over temperature",
  "Bus/battery faults",
  ...
  return (
    <div className="rounded-md border border-white/10 bg-graphite-900 p-4">
      ...
      <div className="mt-4 grid gap-2 text-sm">
        <InfoRow label="CAN feedback" value={feedbackOk ? "Receiving" : "Unknown"} danger={!feedbackOk} />
        <InfoRow label="Enable proof" value="Not exposed by DBC" danger />
        <InfoRow label="Fault word" value={String(faultWord)} danger={activeFaults.length > 0} />
      </div>

      <ProtectionList title="Monitored fault groups" items={[...monitoredProtectionGroups]} empty="None" />
